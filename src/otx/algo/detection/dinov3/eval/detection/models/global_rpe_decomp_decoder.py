...
from otx.algo.detection.dinov3.eval.detection.util.box_ops import box_xyxy_to_cxcywh, delta2bbox
from otx.algo.detection.dinov3.eval.detection.util.misc import _get_activation_fn, _get_clones, inverse_sigmoid
...
class GlobalCrossAttention(nn.Module):
    ...
    def forward(
        ...
        pos_x = (
            torch.linspace(0.5, w - 0.5, w, dtype=ref_pts.dtype, device=ref_pts.device)[None, None, :, None] * stride
        )  # 1, 1, w, 1
        pos_y = (
            torch.linspace(0.5, h - 0.5, h, dtype=ref_pts.dtype, device=ref_pts.device)[None, None, :, None] * stride
        )  # 1, 1, h, 1
...
def build_global_rpe_decomp_decoder(
    d_model = 768,
    d_ffn = 2048,
    dropout = 0.0,
    n_heads = 8,
